...
from dynamax.deep_hidden_markov_model.inference import HMMPosterior, HMMPosteriorFiltered
from dynamax.deep_hidden_markov_model.inference import hmm_filter
from dynamax.deep_hidden_markov_model.inference import hmm_posterior_mode
from dynamax.deep_hidden_markov_model.inference import hmm_smoother
from dynamax.deep_hidden_markov_model.inference import hmm_two_filter_smoother
...
class DeepHMMParameterSet(Protocol):
...
class DeepHMMPropertySet(Protocol):
...
class DeepHMMEmissions(ABC):
    """Abstract class for DeepHMM emissions.
    """
    def __init__(self,
                 m_step_optimizer=optax.adam(1e-2),
                 m_step_num_iters=50) -> None:
    ...
    def compute_means_and_covar(self,
        ...
        """ Apply the emission NN to retrieve the means and the scales of the
        Gaussian emissions. Note that inputs are an mandatory NN input

        Args:
           params: The namedtuple containing the nn_params
           emissions: observed emissions
           inputs: inputs

        Returns:
           a ParameterSet of the same model with populated means and covariance
           related fields
        """
        raise NotImplementedError
...
class DeepHMM(SSM):
    """Abstract base class of Deep Hidden Markov Models (DeepHMMs).
